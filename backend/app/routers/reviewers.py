...
def get_current_reviewer(_session=Depends(check_reviewer), db: Session = Depends(get_db)):
    reviewer = db.get(Reviewer, _session.reviewer_id)
    if not reviewer:
...
def update_current_reviewer(payload: schemas.ReviewerUpdate, db: Session = Depends(get_db), _session=Depends(check_reviewer)):
    reviewer = db.get(Reviewer, _session.reviewer_id)
    if not reviewer:
        raise HTTPException(status_code=404, detail="Reviewer not found!")
